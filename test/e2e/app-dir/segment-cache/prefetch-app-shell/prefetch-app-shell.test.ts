  ...
  describe('root params', () => {
    it('includes root params in a runtime App Shell', async () => {
      ...
      await act(async () => {
        ...
        expect(
          await browser.elementById('root-param-from-layout').text()
        ).toEqual('Root param from layout: en')
        // Sesssion data (cookies) is not dependent on URL-data, so they are
        // allowed to be accessed in the shell.
        expect(await browser.elementById('cookie-value').text()).toEqual(
          'Cookie: none'
    ...
    it('does not re-use runtime App Shells for links with different root params', async () => {
      let page: Playwright.Page
      const browser = await next.browser('/with-root-param/en', {
        beforePageLoad(p: Playwright.Page) {
          page = p
    ...
    it('does not re-use static App Shells for links with different root params', async () => {
      let page: Playwright.Page
      const browser = await next.browser('/with-root-param/en', {
        beforePageLoad(p: Playwright.Page) {
          page = p
